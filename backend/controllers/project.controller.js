...
export const createProject = async (req, res) => {
  try {
    ...
    if (!capital || capital < 1000) {
      return res
        .status(400)
        .json({ message: "Capital must be at least 1000 DH" });
    }
...
export const updateProject = async (req, res) => {
  try {
    ...
    if (project.status === "closed") {
      return res
        .status(400)
        .json({ message: "Cannot update a closed project" });
    }
    ...
    if (maxInvestmentPercent) {
      project.maxInvestmentPercent = Math.min(
        100,
        Math.max(1, Number(maxInvestmentPercent)),
      );
    }
...
export const closeProject = async (req, res) => {
  try {
    ...
    if (project.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Not authorized to close this project" });
    }
...
export const deleteProject = async (req, res) => {
  try {
    ...
    try {
      await Investment.deleteMany({ projectId: id });
      console.log("Related investments deleted");
    } catch (investmentError) {
      console.error("Error deleting investments:", investmentError);
    }
...
export const investInProject = async (req, res) => {
  try {
    ...
    if (project.status === "closed") {
      return res
        .status(400)
        .json({ message: "Project is closed for investments" });
    }

    // Calculer le montant maximum par investisseur
    const maxPerInvestor =
      project.capital * (project.maxInvestmentPercent / 100);
    ...
    const totalInvestedByUser = existingInvestments.reduce(
