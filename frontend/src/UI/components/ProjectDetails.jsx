...
import { ArrowLeft, Target, TrendingUp, Users, DollarSign, Edit2, Trash2, Lock } from 'lucide-react';
import InvestModal from './InvestModal';

const ProjectDetails = ({ project, userRole, balance, onEdit, onClose, onDelete, onInvestSuccess }) => {
      ...
      <div className="details-container">
            ...
            <table className="investors-table">
              ...
              <tbody>
                {project?.investments?.map((inv, idx) => {
                  const percent = (inv.amount / project?.capital) * 100;
                  return (
                    <tr key={inv._id || idx}>
                      <td>{inv.investorId?.name || `Investor ${idx + 1}`}</td>
                      <td className="amount">{inv.amount?.toLocaleString()} DH</td>
                      <td>
                        <span className="percent-badge">{percent.toFixed(2)}%</span>
                      </td>
                    </tr>
                  );
                })}
        ...
        {userRole === 'investor' && isOpen && (
          <div className="invest-section">
            <button onClick={() => setShowInvestModal(true)} className="invest-now-btn">
